...
bool TaskDatumParameters::updatePreview()
{
    ...
    //todo: show message next to superPlacement, if not attached. Maybe enable/disable edit boxes.
    return attached;
}
...
void TaskDatumParameters::updateSuperplacementUI()
{
    ...
    //disable angle edits if rotation part of placement is bound by expressions
    auto expressions = pcDatum->ExpressionEngine.getExpressions();
    bool bRotationBound = false;
    bRotationBound = bRotationBound ||
            expressions.find(App::ObjectIdentifier(pcDatum,std::string("Placement.Rotation.Angle"))) != expressions.end();
    bRotationBound = bRotationBound ||
            expressions.find(App::ObjectIdentifier(pcDatum,std::string("Placement.Rotation.Axis.x"))) != expressions.end();
    bRotationBound = bRotationBound ||
            expressions.find(App::ObjectIdentifier(pcDatum,std::string("Placement.Rotation.Axis.y"))) != expressions.end();
    bRotationBound = bRotationBound ||
            expressions.find(App::ObjectIdentifier(pcDatum,std::string("Placement.Rotation.Axis.z"))) != expressions.end();
