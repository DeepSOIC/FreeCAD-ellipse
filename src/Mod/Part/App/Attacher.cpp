...
int AttachEngine::isShapeOfType(eRefType shapeType, eRefType requirement)
{
    ...
    if (requirement & rtFlagHasPlacement)
        if(! (shapeType & rtFlagHasPlacement))
            return -1;
    //get rid of hasplacement flags, to simplify the rest
    shapeType &= (rtFlagHasPlacement - 1);
    requirement &= (rtFlagHasPlacement - 1);
...
eRefType PartExport operator|=(eRefType &lhs, eRefType rhs)
{
    lhs = eRefType(lhs | rhs);
    return lhs;
}
...
eRefType PartExport operator&=(eRefType &lhs, int rhs)
{
    lhs = eRefType(lhs & rhs);
    return lhs;
}
