...
UnitExpression::UnitExpression(const DocumentObject *_owner, const Base::Unit & _unit, std::string _unitstr, double _scaler)
    : Expression(_owner)
    , unit(_unit)
    , unitstr(_unitstr)
    , scaler(_scaler)
...
void UnitExpression::setUnit(const Unit &_unit, std::string _unitstr, double _scaler)
{
    unit = _unit;
    unitstr = _unitstr;
    scaler = _scaler;
...
{
    return new NumberExpression(owner, 1.0, unit, unitstr, scaler);
}
...
{
    return new UnitExpression(owner, unit, unitstr, scaler);
}
...
NumberExpression::NumberExpression(const DocumentObject *_owner, double _value, const Base::Unit & _unit, std::string _unitstr, double _scaler)
...
{
    return new NumberExpression(owner, value, unit, unitstr, scaler);
}
...
const Property * VariableExpression::getProperty() const
{
    ...
    docObject = doc->getObject(var.getDocumentObjectName().c_str());

    if (docObject == 0)
        docObject = owner;
    else {
        App::Document * doc = owner->getDocument();
        const std::string & object = var.getDocumentObjectName();
        ...
        if (!docObject) {
            ...
            for (std::vector<DocumentObject*>::iterator j = docObjects.begin(); j != docObjects.end(); ++j) {
                if (strcmp((*j)->Label.getValue(), object.c_str()) == 0) {
...
const char * unitstr = 0;                           /**< Variable used to hold current unit string during parsing */
...
std::stack<std::string> labels;                /**< Label string primitive */
...
UnitExpression * ExpressionParser::parseUnit(const App::DocumentObject *owner, const char* buffer)
{
    ...
    if (unitExpression) {
        NumberExpression * num = dynamic_cast<NumberExpression*>(simplified);

        if (num) {
           simplified = new UnitExpression(num->getOwner(), num->getUnit(), num->getUnitString(), num->getValue());
            delete num;
        }
...
App::Path ExpressionParser::parsePath(const App::DocumentObject *owner, const char* buffer)
{
    // parse from buffer
    ExpressionParser::YY_BUFFER_STATE my_string_buffer = ExpressionParser::ExpressionParser_scan_string (buffer);
    ...
    VariableExpression * value = dynamic_cast<VariableExpression*>(ScanResult);
    ...
    if (value) {
        ...
        delete ScanResult;
        throw Expression::Exception("String is not a path.");
        return Path();
    }
}
